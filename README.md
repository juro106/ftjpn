...
## Vim なのだから 日本語編集のときも`f`と`t`を便利に使おう

日本語編集ではあまり使われならしい`f`と`t`にもっと仕事をしてもらおう、というのがこのプラグインの主旨です。
...
let g:ftjpn_key_list = {
    ...
    \ ",": [',', '、', '，'],
    \ "t": ['t', 'と'],
    \ "w": ['を'], "※リストの中に key である 'w' が含まれていなくても、'w' は有効になります。例えば 'hello world' の行の先頭で `fw` を実行すれば 'w' に移動します。常に通常の動作は有効なままです。
    \ }
```
