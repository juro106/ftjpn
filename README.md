...
- `f` `F` `t` `T` を使う操作を統一できます。
- 新たに別なキーの組み合わせを増やさずに済みます。
- 日本語 (マルチバイト文字) に限らず、独自のキー、組み合わせを無数に設定できます。

日本語（マルチバイト文字、全角文字）の編集で`f`や`t`をまともに機能させるには、その都度IMEを切り替えるか、migemo や digraph を使う必要があります。
...
しかし、このプラグインを使えば`f.`で`。`と`.`の両方に移動できるようになります。つまり、英語だろうが日本語だろうが全く同じ動作で仕事が済んでしまうということです。

もちろん「`df.` → `。`まで削除」、「`vt.` → `。`の手前まで選択」といったおなじみの操作も普段と同じように違和感なく使えます。
...
また、キーは必ずしも1対1のペアである必要はなく、1つのキー対して複数の文字を設定することも可能です（便利になるかはさておき）。

`;`に複数の記号を設定しておけば、`f;`と打つだけで色々な記号に移動できるようになり、打ちにくいシフトキーとのペアを打鍵する手間が省けます（記号だらけの行では却って邪魔になることもあると思いますが）。

## 欠点① ノーマルモードの移動を伴うマクロで失敗しても停止しない

ノーマルモードで `ftFT` を使って移動してから何かするようなマクロを組んだ時、通常、指定した文字が見つからなかったときは停止します。

しかし、マクロ実行中に「ノーマルモードでこのプラグインで設定した文字を指定した移動をしている場合」は、指定した文字が見つからなくても停止されません（設定していない文字であれば通常と同じ動作になります)。

一方、オペレーター待機モードやビジュアルモードの場合は通常と同じように停止します。

## 欠点② 英語と日本語が混ざっているときがめんどう

### 例文 1) Hello World. 私は日本人です。東京から来ました。My name is Vim. 本当の名前は太郎です。

カーソルが行頭にあり、`私は日本人です。`の`。`に直接移動したい時。`f.`と打っても`Hello World.`の`.`が選ばれてしまいます。`2f.` の場合は `。` を飛ばして2つ目の `.` に移動します。カーソル位置から最寄りの文字が選ばれるというルールはカウントを使っていても変わりません。
