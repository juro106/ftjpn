...
Vimで日本語を編集する際の`f`,`t`,`F`,`T`を使った移動、削除やヤンク、選択の操作を便利に、かつ質素に使うためのプラグインです。
...
弊プラグインはこのような欠陥を抱えていますが、1行の中に日本語の`。`と英語の`.`がごちゃまぜで書かれていることは稀だと思われます（強いて挙げればこのREADMEぐらいのものではないでしょうか）。
